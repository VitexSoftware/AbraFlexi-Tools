...
$config_file = '/etc/flexibee/client.json';

define('BACKUP_DIRECTORY', sys_get_temp_dir().DIRECTORY_SEPARATOR);
define('EASE_APPNAME', 'FlexiBee Create Company');
...
if ($argc != 2) {
    echo "usage: ".$argv[0]." smazat_firma_a_s_ / https://[user:password@]flexibee.source.cz:5434/c/smazat_firma_a_s_\n";
} else {
    if (strstr($argv[1], '://')) {
    ...
    $source                  = new \FlexiPeeHP\Company($srcOptions['company'],
        $srcOptions);
    ...
    if (is_null($company)) {
        ...
        $source->addStatusMessage(sprintf(_('Company %s removing'),
                $source->getApiURL()),
            $source->deleteFromFlexiBee() ? 'success' : 'error' );
    }
}
